...
export async function getProductBySlug(slug) {
  ...
  try {
    ...
    console.warn(`Falling back to in-memory product data for ${slug}:`, error instanceof Error ? error.message : error);
    return fallbackProductBySlug(slug);
  }
}
...
export async function addCartItem({ cartId, productId, quantity }) {
  ...
  const productRes = await query(
    `select id, name, currency, price_cents, sale_price_cents
     from products
     where id = $1 or (id = $2 and slug = $3)
     limit 1`,
    [productId, heroProduct.id, appConfig.productSlug]
  );
  if (productRes.rows.length === 0) {
  ...
  await query(
    ...
    [randomUUID(), cartId, product.id, quantity, unitPriceCents]
  );
